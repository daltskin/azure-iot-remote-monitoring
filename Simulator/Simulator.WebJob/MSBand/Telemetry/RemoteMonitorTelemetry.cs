...
using Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob.MSBand.Telemetry.Data;
using Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob.SimulatorCore.Logging;
using Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob.SimulatorCore.Telemetry;

namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob.MSBand.Telemetry
{
    public class RemoteMonitorTelemetry : ITelemetry
    {
        ...
        private SampleDataGenerator _speedGenerator;
        ...
        public RemoteMonitorTelemetry(ILogger logger, string deviceId)
        {
            ...
            //_heartRateGenerator = new SampleDataGenerator(40, 180, 200, peakFrequencyInTicks);
            //_speedGenerator = new SampleDataGenerator(0, 5);
            _heartRateGenerator = new SampleDataGenerator(40, 200, 65, peakFrequencyInTicks);
            _speedGenerator = new SampleDataGenerator(0, 5);
        ...
        public async Task SendEventsAsync(CancellationToken token, Func<object, Task> sendMessageAsync)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                if (TelemetryActive)
                {
                    ...
                    monitorData.Speed = _speedGenerator.GetNextValue();
                    messageBody = "HeartRate: " + Math.Round(monitorData.HeartRate, 2)
                        + " Speed: " + Math.Round(monitorData.Speed, 2);
        ...
        {
            _heartRateGenerator.ShiftSubsequentData(newSetPointTemperature);
        }
    }
}